...
public class BinaryTree<T>{
	...
	public int width(){
		...
		MyQueue<BinaryNode<T>> q = new MyQueue<BinaryNode<T>>();
		q.add(root);
	...
	public String breadthFirstTraverse(){
		...
		while(!queue.isEmpty()){
			BinaryNode currentNode = (BinaryNode) queue.remove();
			if(stringBuffer.length() == 0)
				stringBuffer.append(currentNode.getData());
			else
				stringBuffer.append(" " + currentNode.getData()); //need to trim space at end
